...
export class DashboardComponent implements OnInit {
  ...
  */
  getDashboardData(): void{
    this._dataService.getDashboardData(this.githubUser, this.reponame)
    .pipe(takeUntil(this.destroyed$))
    .subscribe(responses => 
      {
        /* Handle responses */
        let repores = responses[0]; //response for repo object
        let commitres = responses[1]; //response for lastcommit object
